...
{   
  ...
  cerr << "System initialized with " << sys.Afree << " KaiA dimers and " << sys.N_hexamers 
       << " KaiC hexamers. " << endl;
  cerr << "Equilibration time: " << sys.tequ << ", Max samples: " << sys.sample_cnt << endl;

  double tswitch_old( sys.tlastswitch );
    
  //Propagate simulation until sample_cnt is reached.
  while( propagate(&sys, hexamers, &prop_cont) ) 
  { 
    /* Record samples for output data */
    if( (sys.tlastswitch > tswitch_old) && (sys.tsim > sys.tequ) )
  ...
  // Output phosphorylation flux per hexamer
  /*
  double temp = 0.;
...
HexamerAvr calc_hex_averages( Hexamer *hexamers, SystemVariables *sys, ReactionConstants *reaction_consts )
{
  ...
  /*cout << sys->tsim << " " <<
       sys->rate_pTp1[0][0] - sys->rate_pTm1[0][0] << " " << 
       sys->rate_pSp1[1][1] - sys->rate_pSm1[1][1] << " " << 
       sys->rate_pTp1[1][1] - sys->rate_pTm1[1][1] << " " << 
       sys->rate_pSp1[0][0] - sys->rate_pSm1[0][0] << " " << 
       sys->CIATPcons << " " <<  sys->CIIATPcons << endl;
  */
...
{ 
  ...
  while (getline(infile, line))
  {
    ...
    {        
      ...
      if( param_name.compare("sample_cnt") == 0 )
      {
        sys->sample_cnt = param_value;
        paramset = 1;
      }  
...
ReactionConstants initialize_reaction_consts(SystemVariables *sys)
{
  ...
  while (getline(infile, line))
  {
    ...
    if (iss >> param_name >> param_value) 
    {
      ...
      if( param_name.compare("kACIADPoff") == 0 )
      {
        reaction_consts.kACIADPoff = param_value; 
        paramset = 1;
      }
      ...
      if( param_name.compare("KACIATPADP") == 0 )
      {
        reaction_consts.KACIATPADP = param_value; 
        paramset = 1;
      }      
