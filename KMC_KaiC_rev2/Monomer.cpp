...
{
  if( reaction_channel > 7 )
  {
    sys->hist_pTVSpS[hexamer->get_hex_T()][hexamer->get_hex_S()] += sys->tsim - hexamer->tlastswitch;
    hexamer->tlastswitch = sys->tsim;
    sys->tlastswitch = sys->tsim;
    /* Register all transitions between U,T,D and S per hexamer
    switch (reaction_channel)
    {
      ...
      break;
      case 12:  sys->rate_pTp1[hexamer->get_hex_T()][hexamer->get_hex_S()]++;
      break;
      case 13:  sys->rate_pTm1[hexamer->get_hex_T()][hexamer->get_hex_S()]++;
      break;
      case 14: sys->rate_pSp1[hexamer->get_hex_T()][hexamer->get_hex_S()]++;
      break;
      case 15: sys->rate_pSm1[hexamer->get_hex_T()][hexamer->get_hex_S()]++;
      break;     
    }
  }
  */
  
  /* Register all transitions between U,T,D and S per monomer */  
    switch (reaction_channel)
    {
      case 8:  sys->rate_pTp1[0][0]++;
      break;
      case 9:  sys->rate_pTm1[0][0]++;
      break;
      case 10:  sys->rate_pSp1[1][1]++;
      break;
      case 11:  sys->rate_pSm1[1][1]++;
      break;
      case 12:  sys->rate_pTp1[1][1]++;
      break;
      case 13:  sys->rate_pTm1[1][1]++;
      break;
      case 14: sys->rate_pSp1[0][0]++;
      break;
      case 15: sys->rate_pSm1[0][0]++;
      break;     
    }
  }
  
  /* Execute state change in monomer */
  switch (reaction_channel)
  {
...
{ 
  ...
  double kACIADPoff ( hexamer->kACIADPoff() );
  
  //Affinity for ATP to ADP in the active state.
  double KCIATPADP ( reaction_consts->KACIATPADP );

  //Affinity for ATP to ADP in the inactive state is calulated by
  // multiplying the active state affinity by the difference in affinity between
  //ATP to ADP afffinity in the inactive and active state.
  if(!hexamer->get_active()) 
    KCIATPADP *= exp(reaction_consts->ddGTDconf);
  ...
  prop_list[0]  = (double) nI * (1. - ATPfrac) * kACIADPoff * KCIATPADP;
  prop_list[1]  = (double) (1-nI) * ATPfrac * kACIADPoff;
